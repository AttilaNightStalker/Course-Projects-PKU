...
def regVars (funcs, dict):

    if isinstance(funcs, list):
        return list(map(lambda x : regVars(x, dict), funcs))

    elif isinstance(funcs, tuple):
        if funcs[0] == "const":
            args = regVars(funcs[1][:1], dict)
            args.extend(funcs[1][1:])
            return tuple([funcs[0], args])
        else:
            return tuple([funcs[0], regVars(funcs[1], dict)])
    
    elif isinstance(funcs, str):
        try:
            return dict[funcs]
        except KeyError:
            dict[funcs] = chr(ord("A") + len(dict))
            return dict[funcs]
...
def dumpData():
    ...
    for sample in prev:
        ...
        parsed = parseExp(input()[:-2])
        regularize(parsed, propDict)
        sample.append(regVars(parsed[1][1], {}))
